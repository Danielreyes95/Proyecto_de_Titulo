...
(async () => {
  try {
    ...
    const data = await api("/invitaciones-entrenador/consultar?token=" + encodeURIComponent(token));
...
$("nuevo").addEventListener("submit", async event => {
  ...
  try {
    const data = await api("/invitaciones-entrenador/aceptar", {
      token, nombre: $("nombre").value.trim(), password: $("password").value
    });
    $("nuevo").hidden = true; $("password").value = "";
    notify(data.mensaje + " Ahora puedes ingresar en el acceso de entrenadores.");
  } catch (error) { notify(error.message); }
...
$("existente").addEventListener("submit", async event => {
  ...
  try {
    ...
    const data = await api("/invitaciones-entrenador/aceptar-existente",
      { token }, session.token);
    $("existente").hidden = true; $("passwordExistente").value = "";
    notify(data.mensaje + ". Ingresa normalmente en el acceso de entrenadores.");
  } catch (error) { notify(error.message); }
  finally { button.disabled = false; }
});
})();
